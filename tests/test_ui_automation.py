...
def test_actions_to_prompts_restart():
    actions = [
    ...
    prompts = actions_to_prompts(actions, service_name="checkout", include_login=False)
    assert "Open the service instances section." in prompts
    assert any("Restart button" in p for p in prompts)
